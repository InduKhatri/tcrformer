...
from transformers.models.electra.modeling_electra import ElectraEncoder, ElectraEmbeddings, ElectraClassificationHead
...
class ElectraTCREmbeddings(nn.Module):
...
class ElectraTCRModel(ElectraPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        if config.task_specific_params['method'] == 2:
            self.embeddings = ElectraTCREmbeddings(config)
        else:
    ...
    def forward(
        ...
        hidden_states = self.embeddings(
            input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids,v_gene_ids = vgenes, j_gene_ids = jgenes, inputs_embeds=inputs_embeds
        )
...
class ElectraTCRClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.method == 1:
            ...
            self.out_proj = nn.Bilinear(config.hidden_size, 2 , config.num_labels)
        else:
            self.dense = nn.Linear(config.hidden_size, config.hidden_size)
            self.dropout = nn.Dropout(classifier_dropout)
    ...
    def forward(self, features, **kwargs):
        ...
        x = self.dropout(x)
        if self.method == 1:
            x = self.out_proj(x, kwargs['genes'])
        else:
            x = self.out_proj(x)
        return x

class ElectraForTCRClassification(ElectraPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.alpha = 1
        self.method = config.task_specific_params['method']
        ...
        if self.method == 2:
            self.electra = ElectraTCRModel(config)
            self.classifier = ElectraTCRClassificationHead(config)
        elif self.method == 1:
    ...
    def forward(
        ...
        if self.method == 1 and vgenes is not None and jgenes is not None:
        # Additional Features gets concatenated to Pooled Output from Transformers
            ...
            arg_genes = {'genes': genes}
            logits = self.classifier(sequence_output, **arg_genes)
...
class ProtTrans(Dataset):
    ...
    def __init__(self, split="train", tokenizer_name='Rostlab/prot_bert', max_length=100):
        ...
        self.tokenizer = ElectraTokenizer.from_pretrained(tokenizer_name,do_lower_case=False)
...
def compute_metrics(pred):
    ...
    plt.savefig('/tudelft.net/staff-umbrella/tcr/md_ele/roc.png', bbox_inches = 'tight')
    experiment.log_image('/tudelft.net/staff-umbrella/tcr/md_ele/roc.png', name='roc', overwrite=False, image_format="png",
    ...
    plt.legend(bbox_to_anchor=(1.04,1), loc="upper left")
    plt.savefig('/tudelft.net/staff-umbrella/tcr/md_ele/prc.png', bbox_inches = 'tight')
    experiment.log_image('/tudelft.net/staff-umbrella/tcr/md_ele/prc.png', name='prc', overwrite=False, image_format="png",
                    image_scale=1.0, image_shape=None, image_colormap=None,
                    image_minmax=None, image_channels="last", copy_to_tmp=True, step=e)
    plt.close()
...
experiment = comet_ml.Experiment(    
    api_key="",
    project_name="electra-tuner-1-SO",
    workspace=""
    )
hyper_parameters = {
    "seed": 57,
    "learning_rate" : 7.789295816796606e-05,
    "gradient_acm": 93,
...
training_args = TrainingArguments(  
    output_dir='/tudelft.net/staff-umbrella/tcr/md_ele/results',
    logging_dir='/tudelft.net/staff-umbrella/tcr/md_ele/logs',
    num_train_epochs=50,
...
def model_init():
    ...
    model = ElectraForTCRClassification.from_pretrained(model_name, 
...
try:
    shutil.rmtree('/tudelft.net/staff-umbrella/tcr/md_ele/results')
    shutil.rmtree('/tudelft.net/staff-umbrella/tcr/md_ele/logs')
...
# study_name = "electra-tuner-1-SO"  # Unique identifier of the study.
